...
bool Socket::shutDownSend()
{
		...
		//When the second parameter is SHUT_RDWR, it will block both sending and receiving (like close). 
	...
	int res = 0;
		...
		res = ::shutdown(socket_, SD_SEND);
		if (socket_ != INVALID_SOCKET)
				return true;
#else
		
		// http://man7.org/linux/man-pages/man2/shutdown.2.html
		res = ::shutdown(socket_, SHUT_WR);
		// both these functions return 0 on success
...
#ifdef _WIN32
		// https://msdn.microsoft.com/en-us/library/windows/desktop/ms740481(v=vs.85).aspx
		res = ::shutdown(socket_, SD_RECEIVE);
		if (socket_ != INVALID_SOCKET)
				return true;
#else
		//http://beej.us/guide/bgnet/html/single/bgnet.html#closedown
...
bool Socket::recv(size_t bytes, byte* buffer)
{
	...
	while (bytesLeft > 0)
	{
		...
		if (socket_ == INVALID_SOCKET || bytesRecvd == SOCKET_ERROR)
			return false;
#else
...
#if _WIN32
		if (bytesSent == INVALID_SOCKET || bytesSent == SOCKET_ERROR)
			return false;
#else
		if (bytesSent <=0) {
			std::cout << "in sendString() - bytesSent = -1" << std::endl;
			return false;
		}
#endif
...
std::string Socket::recvString(byte terminator)
{
	...
	while (true)
	{
		...
		{
#endif
			std::cout << "-- invalid socket in Socket::recvString - iResult = " << iResult << std::endl;
...
size_t Socket::bytesWaiting()
{
	...
	//https://groups.google.com/forum/#!topic/comp.os.linux.development.apps/ZUmyhgLgDLM
	//https://www.ibm.com/support/knowledgecenter/en/SSB23S_1.1.0.14/gtpc2/cpp_ioctl.html
	::ioctl(socket_, FIONREAD, &ret);
#endif
	return (size_t)ret;
}
...
bool SocketConnecter::connect(const std::string& ip, size_t port)
{
	...
	for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
...
#else
			Show::write("\n\n  -- socket failed with error: " + std::string(std::strerror(errno)));
#endif
			return false;
		}
		...
		if (iResult == SOCKET_ERROR) {
			...
			if (iResult == -1) {
				socket_ = INVALID_SOCKET;
				Show::write("\n\n  -- connect failed with error: " + std::string(std::strerror(errno)));
#endif
				continue;
			}
			break;
		...
		if (socket_ == INVALID_SOCKET) {
			...
			return false;
#else
			Show::write("\n -- unable to create socket" + std::string(std::strerror(errno)));
#endif
		}
		return true;
}
...
bool SocketListener::bind()
{
...
#else
		if (iResult == -1) {
			Show::write("\n -- bind failed with error: " + std::string(std::strerror(errno)));
			socket_ = INVALID_SOCKET;
			continue;
		}
#endif
	...
	//https://msdn.microsoft.com/en-us/library/windows/desktop/ms737931(v=vs.85).aspx
...
#else
	if (iResult == -1) {
		Show::write("\n -- listen failed with error: " + std::string(std::strerror(errno)));
		socket_ = INVALID_SOCKET;
		return false;
	}
#endif
