...
std::string Path::getFullFileSpec(const std::string &fileSpec)
{
  ...
  //nk
  //std::cout << "*** - " << getPath(fileSpec.c_str()) << std::endl;
  const std::string fullPath = realpath(getPath(fileSpec.c_str()).c_str(), buffer);
...
Directory::getFiles(const std::string& path, const std::string& pattern)
{
  ...
  while(true)
  {
    ...
    if(dp->d_type == 8)
    {
      ...
      std::string patternSub = pattern.substr(0,pattern.find_last_of(".")-1); 

      if(name.find(patternSub) != std::string::npos && match(name,pattern)) {
//	std::cout << "*** here-4 name :: pattern = " << name << " :: " << pattern << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  if(argc > 1)
    ...
    fn1 = std::string(getenv("HOME")) + "/src/DO_Project/5-3/newCode/FileSystem/FileSystem.h";
  ...
  {
    fn1 = std::string(getenv("HOME")) + "/src/DO_Project/5-3/newCode/FileSystem/FileSystem.h";
    fn2 = std::string(getenv("HOME")) + "/src/DO_Project/5-3/newCode/FileSystem/FileSystem.cpp";
  }
  FileInfo fi1(fn1);
  FileInfo fi2(fn2);
  ...
  std::string srcFile = "/home/nachiket/src/DO_Project/5-3/newCode/FileSystem/libFileSystem.so";
  std::string src = srcFile;
  std::cout << "\n  src = \"" << src << "\"";
